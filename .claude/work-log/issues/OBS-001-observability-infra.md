...
JVM 스레드, DB 커넥션 풀, HTTP 지표, 커스텀 캐시 메트릭을 실시간으로 시각화할 수 있는 환경이다.
부하 테스트(Phase 2) 중 병목 지점을 Grafana에서 실시간 확인하기 위해 도입했다.

## 발견 배경
부하 테스트를 수행하기 전 Tomcat 스레드 포화, DB 커넥션 풀 고갈, 캐시 히트율 등을 실시간으로
...
- `monitoring/grafana/dashboards/backend.json`: JVM / HTTP / 캐시 커스텀 대시보드
...
## 결과 (After)
- Grafana 대시보드에서 JVM 스레드 수, DB active 커넥션, HTTP P95, 캐시 히트율 실시간 확인 가능
...
## 이력서 포인트
Spring Boot Actuator → Prometheus → Grafana 파이프라인을 직접 구성해 부하 테스트 중 JVM 스레드·DB 커넥션 풀·캐시 히트율을 실시간 시각화하는 관측 가능성 인프라를 구축했다.
