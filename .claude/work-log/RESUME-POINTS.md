...
- Caffeine(L1, 5분/500개) + Redis(L2, 60분) 이중 캐시와 Redis SET NX 기반 Stampede 방지를 직접 설계해 동일 질문의 LLM(RAG) 재호출을 차단, 캐시 워밍업 후 L1 히트 200/s · L1 히트율 58.2% · Stampede 차단 13.2/s 달성 및 캐시 히트 시 RAG 응답 5초 → 수 ms 수준으로 단축 (OBS-004)
...
- Spring Boot Actuator → Prometheus → Grafana 파이프라인을 직접 구성해 부하 테스트 중 JVM 스레드 · DB 커넥션 풀 · 캐시 히트율을 실시간 시각화하는 관측 가능성 인프라를 구축하고, Tomcat 스레드 포화 임계(VU 200) 시점을 Grafana에서 직접 관찰했다 (OBS-001)
...
- L1/L2 히트율 · Stampede 빈도 · Circuit Breaker 상태 · 배치 실행 시간·처리 건수 전 계층의 운영 지표를 Prometheus 커스텀 메트릭(Counter/Timer/Gauge)으로 노출해 Grafana 대시보드에서 단일 화면으로 모니터링 가능하게 했다 (OBS-004, OBS-005)
...
- POST /api/v1/query 처리 경로에 캐시(L1/L2) → 멱등성 처리 → Circuit Breaker를 계층적으로 적용해 외부 LLM 의존도를 낮추는 방어적 설계를 구현하고, 캐시 키를 정규화(trim · 연속공백 제거 · 소문자) 후 SHA-256 해시로 구성해 의미상 동일한 질문이 같은 캐시 항목을 공유하도록 설계했다 (OBS-004)
